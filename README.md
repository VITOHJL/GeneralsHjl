...
### 可在 [在线试玩](http://generals.zech.xin)，我们已上传所有agent
...
│   │       ├── MinimaxAI.js # Minimax算法AI
│   │       ├── StrategyRLAI.js # 策略强化学习AI
│   │       ├── QLearningAI.js  # 基础Q-Learning AI
│   │       ├── StrategyExecutors.js # RL策略执行器（BFS、多源攻击等）
│   │       └── index.js     # AI导出
│   ├── training/            # AI训练和评估系统
│   │   ├── train.js         # 命令行工具（评估、对比、基准测试）
│   │   ├── train_rl.js      # RL训练命令行工具
│   │   ├── train_qlearning.js # QLearningAI专用训练脚本
│   │   ├── RLTrainer.js     # RL训练器
...
3. **Q表文件**：确保 `qtable-strategy.json` 和 `qtable-qlearning.json` 在 `dist/` 目录中
...
- [x] **MinimaxAI** - Minimax算法AI（博弈树搜索）
  ...
  - 适合人工智能课程学习和研究
- [x] **StrategyRLAI** - 策略强化学习AI（分层Q-Learning）
  - 使用 Q-Learning 学习高层策略选择（ATTACK、DEFEND、GROW、ALL_IN）
  - 底层策略执行由启发式算法提供（BFS路径规划、多源攻击、主动防御等）
  - 状态编码：压缩游戏状态为5维特征（领土、兵力、距离、要塞、回合）
  - 奖励函数：早期胜利奖励、领土损失惩罚
  - 训练后可达到高胜率（1v1对adaptive约92%,1v1对random约100%，1v3对adaptive约75%）
- [x] **QLearningAI** - 基础Q-Learning AI
  - 直接学习动作级别的Q值
  - 状态编码：2维特征（领土、兵力）
  - 适合研究基础强化学习算法
...
- [x] 支持2-8个玩家(理论无上限)
- [x] 每个玩家可配置为人或AI
- [x] AI类型选择（Random、Adaptive、Minimax、策略RL AI、Q-Learning AI）
- [x] 地图尺寸选择（小15x15、中25x25、大35x35）
- [x] RL AI自动加载训练好的Q表（前端通过fetch加载）
...
### StrategyRLAI（策略强化学习AI）

基于分层架构的强化学习AI，使用 Q-Learning 学习高层策略选择，底层策略执行由启发式算法提供。

#### 设计理念

- **分层架构**：将复杂的游戏决策分解为高层策略选择（RL）和底层战术执行（启发式）
- **状态空间压缩**：将游戏状态压缩为5维特征，避免状态空间爆炸
- **策略抽象**：学习选择4种高层策略（ATTACK、DEFEND、GROW、ALL_IN），而不是学习具体动作
...
1. **状态编码**：
   ...
   - 要塞数量（3档）
   - 回合数阶段（3档）
   - 总状态空间：20×20×5×3×3 = 18,000

2. **Q-Learning参数**：
   - 学习率：0.1
   - 折扣因子：0.95
   - ε-贪婪探索：初始0.1，衰减到0.05

3. **奖励函数**：
   - 胜利：+1000（早期胜利额外奖励）
   - 失败：-50（领土损失惩罚）
   - 避免maxTurns判定：不学习"人口胜利"策略

4. **底层策略执行器**（StrategyExecutors）：
   - **BFS路径规划**：避山寻路，支持多步移动
   - **多源攻击**：协调多个格子同时攻击高价值目标
   - **主动防御**：检测边界入侵、背刺风险，提前防御
   - **后勤运输**：从后方向前线输送兵力
   - **关键位置保护**：多人游戏中保护首都/要塞的最小兵力阈值

#### 训练方法

```powershell
# 训练 StrategyRLAI（对 random 对手）
...
直接学习动作级别的Q值，适合研究基础强化学习算法。

#### 特点

- **简化状态空间**：2维特征（领土占比、兵力规模）
- **动作空间**：当前简化为"random"动作（实际执行随机移动）
- **适合学习**：理解Q-Learning的基本原理
...
| 特性 | StrategyRLAI | QLearningAI |
|------|-------------|-------------|
| 状态编码 | 5维（18,000状态） | 2维（400状态） |
| 动作空间 | 4种策略 | 1种动作（random） |
| 底层执行 | 启发式算法 | 随机移动 |
| 学习目标 | 策略选择 | 动作选择 |
| 适用场景 | 实际对战 | 算法学习 |
...
使用 `train.js compare` 命令可以对比不同 AI 的性能：

```powershell
# 对比 StrategyRLAI 和 QLearningAI
...
**注意**：使用 RL AI 时，需要通过 `--qtablePN` 参数指定 Q 表文件路径（N 为玩家编号）。
...
- `--agent`: AI类型（`strategy` 或 `qlearning`）
- `--episodes`: 训练轮数
- `--players`: 玩家数量
- `--width/--height`: 地图尺寸
- `--maxTurns`: 最大回合数
- `--mode`: 训练模式（`self-play`、`against-fixed`、`mixed`）
- `--opponent`: 对手类型（`random`、`adaptive` 等）
- `--savePath`: Q表保存路径
- `--loadPath`: 加载已有Q表继续训练
- `--saveInterval`: 每隔多少轮保存一次
- `--logInterval`: 每隔多少轮输出进度

#### 前端使用训练好的模型

训练好的Q表文件会自动加载到前端：

1. **部署Q表文件**：将Q表文件复制到 `public/` 目录
2. **配置映射**：在 `src/store/gameStore.js` 中配置Q表文件路径
3. **选择AI类型**：在游戏菜单中选择"策略RL AI"或"Q-Learning AI"
4. **自动加载**：游戏初始化时自动通过fetch加载Q表

当前已配置的Q表：
- `StrategyRLAI` → `/qtable-strategy.json`
- `QLearningAI` → `/qtable-qlearning.json`
...
**📖 RL策略文档请查看：[src/training/RL_STRATEGY.md](./src/training/RL_STRATEGY.md)**

**📖 项目技术报告请查看：[PROJECT_REPORT.md](./PROJECT_REPORT.md)**
